...
def insert_entry(title, post, tags_array, author):
    ...
    try:
        ...
        post_id = db.posts.find_one({'permalink': permalink})
        if not post_id:
...
def post_newcomment():
    ...
    if (name == "" or body == ""):
        ...
        try:
            ...
            posts.update({'permalink':post.get('permalink')}, {'$push': {'comments':comment}})
#            posts.find_and_modify(, {'$set': {comment: self.request.POST[field]}}
            # your update here.
